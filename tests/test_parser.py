...
def test_detects_multiple_lesson_types_in_one_cell():
    text = (
        "Тех. и алгор. высокопроизводительных вычисл. ПРАКТ.\n"
        "Замятина Е.Б. (511[3], 1)\n"
        "Разработка промышленных систем на языке Go (семинары)\n"
        "Городилов А.Ю. (503[3], 2)"
    )
...
def test_keeps_literal_subgroup_number_from_schedule():
    entries = parse_lesson_entries(
        "Базы данных\nКоробков И.И. (онлайн[0], 7)\nhttps://example.test"
    )
    assert entries[0][-1] == 7
